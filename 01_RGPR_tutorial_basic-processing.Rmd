...
```{r plot, echo=TRUE, fig.cap = 'plot(A)'}
plot(A)                                 
```
...
Plot wiggles
```{r plot_wiggles, echo=TRUE, fig.cap = 'plot(A) with wiggles'}
plot(A, type = "wiggles")          
```
...
```{r plot_xlim_ylim, echo = TRUE, fig.cap = 'plot(A) with xlim and ylim'}
...
```{r relTime0, echo = TRUE, fig.cap = 'plot(A) relative to time0'}
...
```{r depth_pos, echo=TRUE, eval = FALSE}
depth(A)     
pos(A)
```

Plot a section/subset of the GPR record (like zooming)
```{r plot_subset, echo=TRUE, fig.cap = 'plot(A) subset'}
...
(between $0$ and $20\,ns$):
```{r plot1D, echo=TRUE, fig.cap = 'plot single trace'}
...
```{r plot1D_subset, echo=TRUE, fig.cap = 'plot single trace, fist 40 samples'}
...
```{r plot_help, echo=TRUE, eval=FALSE}
?plot.GPR
```
...
## DC shift removal
Plot a single trace:
```{r dcShift_plot1D_15, echo=TRUE, fig.cap = 'plot single trace'}
plot(A[, 15])  # plot the 15th trace of the GPR-line
```
...
```{r dcShift_plot_first_samples, echo=TRUE, fig.cap = 'plot single trace, first 110 samples'}
...
```{r dcShift_check_results_1D, echo=TRUE, fig.cap = 'plot single trace + dc-shift'}
plot(A[, 15])  # plot the 15th trace of the GPR-line 
...
Have a look at A1:
```{r dcShift_check_results_2D, echo=TRUE}
A1
```
...
```{r first_wave_break, eval=TRUE, fig.cap = 'plot first wave break time'}
...
```{r time0_check, echo=TRUE, fig.cap = 'plot single trace with time0 and first wave break time'}
...
```{r time0Cor, eval=TRUE}
A2 <- time0Cor(A1, method = "pchip")
```


```{r time0Cor_check, eval=TRUE, fig.cap = 'plot after time0Cor()'}
plot(A2)
```
...
```{r dewow, echo=TRUE,message=FALSE, fig.cap = 'plot after dewow'}
...
```{r dewow_diff, echo=TRUE,message=FALSE, fig.cap = 'plot difference after dewow'}
...
(red line):
```{r dewow_check_1D, echo=TRUE,message=FALSE, fig.cap = 'plot single trace dewow'}
...
```{r fFilter_spectrum, echo=TRUE, fig.cap = 'plot spectrum'}
spec(A3)
```
...
the filter.
```{r fFilter_fFilter, echo=TRUE, fig.cap = 'plot frequency filter'}
A4 <- fFilter(A3, f = c(150, 260), type = "low", plotSpec = TRUE)
plot(A4)
```

Let see the difference
```{r fFilter_diff, echo=TRUE, fig.cap = 'plot difference'}
plot(A4 - A3, clip = 50)
```
...
```{r plotAmpl, echo=TRUE, fig.cap = 'plot amplitude'}
...
```{r gain_exp, echo=TRUE, fig.cap = 'plot ampliude exponential gain'}
A6 <- gain(A5, type ="exp",  alpha = 0.2, t0 = 0, te = 125)
plotAmpl(A6, col = "green")
```

Oops! Set `alpha` to a smaller value!
```{r gain_check2, echo=TRUE, fig.cap = 'plot amplitude after exponential gain'}
...
the argument `clip`:
```{r gain_diff, echo=TRUE, fig.cap = 'plot after gain'}
...
```{r invnorm, echo=TRUE, fig.cap = 'plot hist'}
hist(A6[], breaks = 50)
```
...
```{r traceScaling, echo=TRUE}
A7 <- traceScaling(A6, type = "invNormal")
```

Histograms before and after

```{r hist_diff, echo=TRUE, fig.cap = 'plot histogram comparison'}
par(mfrow = c(1, 2))
hist(A6[], breaks = 50)
hist(A7[], breaks = 50)
```
Have a look at the results of the transformation:
```{r invnorm_check, echo=TRUE, fig.cap = 'plot inverse normal transformation results'}
plot(A7)
```

## Median filter (spatial filter)
A non-linear filter to remove noise:
```{r median_filter, echo=TRUE, fig.cap = 'plot median filter'}
A8 <- filter2D(A7, type = "median3x3")
plot(A8)
```

Let see the difference
```{r median_filter_diff, echo=TRUE, fig.cap = 'plot difference after median filter'}
plot(A8 - A7)
```
...
```{r fkspec, echo=TRUE, fig.cap = 'plot fk-filter'}
...
```{r fk_plot, echo=TRUE, fig.cap = 'plot fk-spectrum'}
plot(A9, clip = 50)
spec(A9, type = "f-k")
```

Let see the difference
```{r fK_diff, echo=TRUE, fig.cap = 'plot difference after fk-filter'}
plot(A9 - A8)
```

## Processing overview
Let review the processing step applied on the GPR record:
```{r proc_overview, echo=TRUE}
proc(A9)
```
...
```{r help_trace_average, echo=TRUE, eval = FALSE}
?traceAverage
```
