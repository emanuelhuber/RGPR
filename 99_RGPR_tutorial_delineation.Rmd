---
title: "RGPR tutorial - Delineation on GPR data"
...
* Learn how to delineate structures on GPR data.

However, this tutorial will not explain you any sedimentology approach to delineate subsurface structures on GPR data.
...
* Process the data [RGPR - Basic GPR data processing (tutorial 2)](http://emanuelhuber.github.io/RGPR/RGPR_tutorial_basic-processing.html)
...
```{r, echo=TRUE}
A3 <- gain(A2, type = "power", alpha = 1, te = 200, tcst = 100)
```
...
between \(50\,ns\) (`t0`) and \(200\,ns\) (`te` for \(t_{end}\)  ):
```{r, echo=TRUE}
A4 <- gain(A3, type ="exp",  alpha = 0.2, t0 = 50, te = 200)
plotAmpl(A4, col = "green")
```

Oops! Set `alpha` to a smaller value!
```{r, echo=TRUE}
A4 <- gain(A3, type = "exp", alpha = 0.09, t0 = 50, te = 180)
plotAmpl(A4, col = "green")
