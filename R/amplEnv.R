...
#' @param method (`character[1]`) Method to use. See details.
#' @param npad (`integer[1]`) Only for \code{method = "hilbert"}. Positive 
...
#'             transform).
#' @param threshold (`numeric[1]`) Threshold value for peak detection. The
...
#' @rdname amplEnv
#' @export
setGeneric("amplEnv", function(x, method = c("peak", "hilbert"),
                                npad = 100, threshold = 2) 
  standardGeneric("amplEnv"))


#' @rdname amplEnv
#' @export
setMethod("amplEnv", "GPR", function(x, method = c("peak", "hilbert"),
                                      npad = 100, threshold = 2){
  #if(is.null(FUN)){
...
getAmplLocalMax <- function(x, threshold = 2){
  xabs <- abs(x@data)
  x@data <- apply(xabs, 2, .intpLocalMaxAmpl, x = x@z, threshold = threshold)
  return(x)
}
