...
#'          
#' @references
#' * Modified Coppens method: Sabbione J.I. and Velis D. (2010) 
#'        Automatic first-breaks picking: New strategies and algorithms. 
#'        Geophysics, 75(4): 67-76.`
#' * Modified Energy Ratio (MER) method: Han L., Wong J., and John C. 
...
setMethod("pickFirstBreak", 
          ...
          function(x, method = c("coppens","threshold",  "MER"), 
            ...
            if(method == "coppens"){
              dz <- mean(diff(x@z))
              if(is.null(w)) w <- abs(diff(range(x@z)))/10
              w <- round(w / dz)
              if( (w %% 2) == 0 ) w <- w + 1
              ...
              #  fb <- .pickFirstBreakModCoppens2(xs, w = w, ns = ns, bet = bet)
              # below: the not vectorised version of Coppens...
              # fb <- apply(x@data[1:nmax, , drop = FALSE], 2, .pickFirstBreakModCoppens, w = w, ns = ns, 
              fb <- apply(x@data^2, 2, .pickFirstBreakModCoppens, w = w, ns = ns, 
                          bet = bet, shorten = shorten)
              fb <- x@z[fb] # fb * x@dz
            }else if(method == "threshold"){
              if(is.null(thr)) thr <- 0.1
              thres <- thr * max(x)
              fb <- apply(abs(x@data), 2, .pickFirstBreakThres, thr = thres, 
                          x@z)
            }else if(method == "MER"){
              dz <- mean(diff(x@z))
              if(is.null(w)) w <- abs(diff(range(x@z)))/10
              w <- round(w / dz)
              # w <- round(w / x@dz)
              # fb <- .pickFirstBreakMER(x@data[1:nmax, , drop = FALSE], w = w,
              fb <- apply(x@data^2, 2, .pickFirstBreakMER, w = w, shorten = shorten)
              fb <- x@z[fb]
            }
            if(any(is.na(fb))){
...
.pickFirstBreakMER <- function(x, w, shorten = TRUE){
  if(shorten == TRUE){
    nmax <- which.max(x)
...
# .pickFirstBreakMER <- function(x, w){
...
.pickFirstBreakThres <- function(x, thr = 0.12, tt){
...
# .pickFirstBreakModCoppens2 <- function(x, w = 11, ns = NULL, bet = 0.2){
#   if(is.null(ns)){
#     ns <- 1.5 * w
#   }
#   
...
.pickFirstBreakModCoppens <- function(x, w = 11, ns = NULL, bet = NULL, shorten = TRUE){
  # if(is.null(ns)){
  #   ns <- 1.5 * w
  # }
