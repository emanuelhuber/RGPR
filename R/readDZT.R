...
readDZT <- function(dsn) {
  ...
  if (!inherits(dsn, "connection")) dsn <- file(dsn, "rb")
...
#' Read GSSI GPS data (.dzg)
#'
#' Parses NMEA GSSIS+GPGGA sentence pairs from a GSSI .dzg file and returns
#' a data frame of georeferenced trace positions.
#'
#' @param dsn (`character(1)|connection`) Path or open connection to the .dzg
#'            file.
#' @return A data frame with columns \code{x}, \code{y}, \code{z} (coordinates
#'         in decimal degrees), \code{id} (1-based trace index), and
#'         \code{time} (UTC time).
#'
...
readDZG <- function(dsn) {
  ...
  if (sum(test_gssis) != sum(test_gpgga)) {
    stop("File '.dzg' is corrupted: unequal GSSIS / GPGGA sentence counts.",
         call. = FALSE)
  }
  
  pat_gssis <- "\\$(?<ID>GSSIS),(?<tr>[0-9]+),(?<time>[-]?[0-9.]+)"
  pat_gpgga <- paste0(
    "\\$(?<ID>GPGGA),(?<UTC>[0-9.]+),(?<lat>[0-9.]+),",
    "(?<NS>[NS]),(?<lon>[0-9.]+),(?<EW>[EW]),(?<fix>[0-9]),",
    "(?<NbSat>[0-9.]+),(?<HDOP>[0-9.]+),(?<H>[0-9.]+),",
    "(?<mf>[MmFf]+)"
  )
  
  gpgga <- extractPattern(x[test_gpgga], pattern = pat_gpgga, start = 0, stop = -1)
  gssis <- extractPattern(x[test_gssis], pattern = pat_gssis, start = 0, stop = -1)
  
  dim(gpgga) <- c(sum(test_gpgga), 11L)
  gpgga      <- as.data.frame(gpgga, stringsAsFactors = FALSE)
  colnames(gpgga) <- c("ID", "UTC", "lat", "NS", "lon", "EW",
                       "fix", "NbSat", "HDOP", "H", "mf")
  
  dim(gssis) <- c(sum(test_gssis), 3L)
  gssis      <- as.data.frame(gssis, stringsAsFactors = FALSE)
  colnames(gssis) <- c("ID", "trace", "time")
  
  xyzt <- .getLonLatFromGPGGA(gpgga)
  
  # Trace numbers in the file start at 0 — convert to 1-based
  mrk        <- cbind(xyzt[, 1:3], as.integer(gssis$trace) + 1L, xyzt[, 4])
...
.read_dzt <- function(dsn, fName, fPath, desc, Vmax, verbose, ...) {
  ...
  if (!is.null(dsn[["GPS"]])) {
    x_gps <- verboseF(readDZG(dsn[["GPS"]]), verbose = verbose)
  }
