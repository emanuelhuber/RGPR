...
readDT <- function(dsn, endian = endian){
  ...
  hd$file_version <- readBin(dsn, what = integer(), n = 3L, size = 1, endian = endian)
  hd$len_rec <- readBin(dsn, what = integer(), n = 1L, size = 2, endian = endian)
  ...
  # seek(dsn, where = NA)
  while(substr((u <- readBin(dsn, what = character(), n = 1L, size = 1, endian = endian)),1 ,1) != "R"){
    ...
    if(substr(u, 1, 2) == "FI"){
      ...
      hd$survey_info <- readBinChar(dsn, n = hd$len_rec - 4, size = 1, endian = endian)
    }else if(substr(u, 1, 1) == "C"){
      hd$survey_info <- readBinChar(dsn, n = hd$len_rec - 4, size = 1, endian = endian)
    }else if(substr(u, 1, 2) == "AH"){
      hd$height <- ascii2num(dsn, 16)
    }else if(substr(u, 1, 2) == "FZ"){
      hd$zone <- readBinChar(dsn, n = hd$len_rec - 4, size = 1, endian = endian)
      ...
      hd$tx_n   <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$tx_seq <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$rx_n   <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$rx_seq <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$nacq   <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
    }else if(substr(u, 1, 2) == "AM"){
      hd$direct <-  readBinChar(dsn, n =1, size = 1, endian = endian)
      ...
      hd$info <- trimStr(readBinChar(dsn, n = hd$len_rec - 4, size = 1, endian = endian))
    }else if(substr(u, 1, 1) == "S"){
      hd$S <- readBinChar(dsn, n = hd$len_rec - 4, size = 1, endian = endian)
    }else if(substr(u, 1, 2) == "FW"){
      hd$da_buttare <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      #= fread(fid,1,'integer*4');
      hd$channel_n <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      # dati.n_canali = fread(fid,1,'integer*4');
      hd$stacking <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      # dati.stacking = fread(fid,1,'integer*4');
      hd$interleaving <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      # dati.interleaving = fread(fid,1,'integer*4');
      hd$channel_id <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      # dati.id_canale = fread(fid,1,'integer*4');
      hd$SOS_high <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      # dati.SOS_high = fread(fid,1,'integer*4');
      hd$sampling_max <- ascii2num(dsn, 16)
      # dati.max_sampling_AD =ascii2num(fid,16);
      hd$sw_version <- readBinChar(dsn, n = 10, size = 1, endian = endian)
      # temp = fread(fid,10,'char*1');
      # dati.SW_version = char(temp');
      hd$build_version <- readBinChar(dsn, n = 10, size = 1, endian = endian)
      # temp = fread(fid,10,'char*1');
      # dati.build_version = char(temp');
      hd$fw_version <- readBinChar(dsn, n =7, size = 1, endian = endian)
      ...
      hd$scratch <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$n_acq_sweep <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$n_acq_sample <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$n_sampler_x <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$n_sampler_y <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$enable_x_compress <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$n_x_compress <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$n_y_compress <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$enable_wheel <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$wheel_compress <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      hd$ad_offset <- readBin(dsn, what = integer(), n = 1, size = 4, endian = endian)
      ...
      hd$symbol <- readBinChar(dsn, n = hd$len_rec - 4, size = 1, endian = endian)
      ...
      hd$info_operation <- readBinChar(dsn, n = hd$len_rec - 4, size = 1, endian = endian)
    }else if(substr(u, 1, 2) == "FN"){
      hd$id_sample_noise <- readBin(dsn, what = integer(), n = 1L, size = 4, endian = endian)
  ...
  for(i in seq_len(hd$n_sampler_x)){
    DDhd1[i] <- readBin(dsn, what = integer(),  n = 1, size = 2, endian = endian)
    DDhd2[i] <- readBin(dsn, what = integer(),  n = 1, size = 2, endian = endian)
    DD[, i] <- readBin(dsn, what = integer(),  n =  hd$n_sampler_y, size = 2, endian = endian)
  }
  .closeFileIfNot(dsn)
