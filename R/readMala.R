...
#
# Mandatory : *.rd3 OR *.rd7 (data), *.rad (header)
# Optional  : *.cor (GPS/position coordinates)
#
...
#'
#' @param dsn     Named list.  The data slot is keyed as \code{RD3} or
#'                \code{RD7} (whichever extension was supplied).  Additional
#'                slots: \code{RAD} (mandatory), \code{COR} (optional).
#' @param fName   (`character(1)`) Base filename of the primary data file.
#' @param fPath   (`character(1)`) Full path of the primary data file.
...
.read_rd3 <- function(dsn, fName, fPath, desc, Vmax, verbose, ...) {
  ...
  is_rd7  <- !is.null(dsn[["RD7"]])
  nbytes  <- if (is_rd7) 4L else 2L
  data_slot <- if (is_rd7) "RD7" else "RD3"
  
  rad  <- verboseF(readRAD(dsn[["RAD"]]), verbose = verbose)
  rd37 <- verboseF(readRD37(dsn[[data_slot]],
                            ntr    = rad$ntr,
                            npt    = rad$npt,
                            nbytes = nbytes),
                   verbose = verbose)
  ...
  x <- verboseF(
    .gprRD3(list(hd = rad$HD, data = rd37),
            fName = fName, fPath = fPath,
            desc  = desc, nbits = 8L * nbytes, Vmax = Vmax),
    verbose = verbose
  )
...
register_gpr_format(
  id         = "RD3",
  detect_ext = "RD3",
  mandatory  = c(RD3 = "RD3", RAD = "RAD"),
  optional   = c(COR = "COR"),
  gps_ext   = "COR",
  reader_fn  = .read_rd3
)

# RD7 (32-bit) variant — same reader, different primary extension
register_gpr_format(
  id         = "RD7",
  detect_ext = "RD7",
  mandatory  = c(RD7 = "RD7", RAD = "RAD"),
  optional   = c(COR = "COR"),
  gps_ext   = "COR",
  reader_fn  = .read_rd3
)
