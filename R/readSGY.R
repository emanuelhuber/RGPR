...
# SEG-Y  —  SGY / SEGY
#
# Covers both classical SEG-Y files and the Radar Systems, Inc. (Prism/Zond)
# variant, which is auto-detected by inspecting the textual file header.
#
# Mandatory : *.sgy OR *.segy
# Optional  : (none)
# =============================================================================


#' Read a SEG-Y GPR file (.sgy / .segy)
#'
#' Format-specific reader called by the dispatcher.  Not intended to be called
#' directly by users; use \code{\link{readGPR}} instead.
#'
#' The reader automatically distinguishes between classical SEG-Y and the
#' Radar Systems, Inc. (Prism / Zond GPR) variant by inspecting the textual
#' file header for the strings \code{"Prism"} and \code{"Radar Systems, Inc."}.
#'
#' @param dsn     Named list.  The data slot may be keyed as \code{SGY} or
#'                \code{SEGY} depending on which extension was supplied.
#' @param fName   (`character(1)`) Base filename of the primary .sgy/.segy file.
#' @param fPath   (`character(1)`) Full path of the primary file.
...
.read_sgy <- function(dsn, fName, fPath, desc, Vmax, verbose, ...) {
  ...
  # Open a connection if a character path was supplied
...
# -----------------------------------------------------------------------------
# Format registration  (both common extensions point to the same reader)
# -----------------------------------------------------------------------------
register_gpr_format(
  id         = "SGY",
  detect_ext = "SGY",
