...
#' @rdname velLayers
#' @export
setGeneric("velLayers", function(x, v, twt = NULL, method = "pchip", 
                                          clean = TRUE) 
...
#' @rdname velLayers
#' @export
setMethod("velLayers", "GPR", function(x, v, twt = NULL, 
  ...
  # twt
  dz = mean(diff(x@z))
  x_del <- (apply(twt, 1, .twt_to_delineations, dtt = dz))
  ...
  x@md[["velocity_interfaces"]] <- unlist(x_del, recursive = FALSE)
