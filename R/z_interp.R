...
# xy = (x, y)-coordinates
# output
#    i = index of all coordinates for which u has been interpolated
#    u = the interpolated u values
interpToCoords <- function(i, u, xy, method = "linear"){
  rmDpl <- diff(RGPR::pathRelPos(cbind(i, u))) == 0
  # i = vl
  # u = y
  i <- i[!rmDpl]
  u <- u[!rmDpl]
  ...
  for(j in 1:length(isplt)){
    ...
    if(length(k) > 1){
      ...
      v <- c(v, k[l+1])
      dxyk <- RGPR::pathRelPos(xy[v, ]) #cumsum(c(0, abs(diff(xi[v]))))
      dxyj <- dxyk[kdx]
      uj <- usplt[[j]]
